function TTLEM_Compare_Fixed()
	% TTLEM experiments with a fixed drainage network
...
function run_exp(ui,uf,ts,nm,prefix);
	...
	% p.resultsdir=[upper(prefix) '_' num2str(ts) '_SS'];
	...
	%%% TRANSIENT 
	load([upper(prefix) '_' num2str(ts) '_SS' filesep prefix '_' num2str(ts) '_ss_100000000.mat'],'H1');
	...
	%%%% FIX DRAINGE DIRECTIONS
	p.DrainDir='fixed';

	% Model Specifics 
	p.resultsdir=[upper(prefix) '_' num2str(ts) '_TR_FIX'];
